...
- **الاستخدام:** الصفحات تربط `assets/css/tailwind.css` (ناتج مُترجَم ومحفوظ في المستودع، لذلك تعمل الصفحات بالنقر المزدوج بدون أي أدوات) **وأيضاً** Tailwind CDN (`https://cdn.tailwindcss.com`) مع الإعداد المشترك `tailwind.config.js`، فأي class جديد تكتبه في الـ HTML يعمل مباشرة في المتصفح بدون `npm run build`. ملف الـ CSS المُترجَم يبقى للـ reset والمتغيرات ولتعمل الصفحات بدون إنترنت. (الـ CDN يعرض في الـ Console تحذير "غير مخصص للإنتاج" وهذا متوقع.)
...
│   │   ├── upload.js           (مناطق رفع الصور/الملفات داخل النوافذ: نقر أو سحب وإفلات + معاينة + استبدال/إزالة)
│   │   ├── learning.js         (صفحات «التعلم»: بحث/فلترة الجداول، طيّ شجرة التصنيفات، عناصر الاختيار)
│   │   ├── dashboard.js        (تفاعل الرسم البياني في لوحة السوبر أدمن فقط)
│   │   ├── user-table.js       (بحث/فلاتر/أعمدة/تحديد لقوائم المستخدمين — خانات اختيار حقيقية مع حالة «جزئي» لرأس العمود)
...
### التعلم (مجموعة «التعلم» في السايدبار)

كل صفحة قائمة + صفحات عرض (Workspace بتبويبات) + نوافذ منبثقة ثابتة (تصميم فقط تُفتح وتُغلق)، ومأخوذة كما هي من الـ Prototype:

- **[admin/waitlist.html](admin/waitlist.html)** — قائمة الانتظار: شريط KPI، بحث وفلاتر الحالة (قيد المراجعة / بانتظار المستندات / مقابلة مجدولة)، جدول 7 طلبات بإجراءات (عرض، موافقة، رفض، طلب مستندات، إسناد) تفتح حوارات تأكيد.
  و**admin/waitlist/request-1…7.html** — صفحة الطلب بخمسة تبويبات (نظرة عامة، معلومات الطلب، المستندات، سجل CRM، الأنشطة).
- **[admin/placement-exam.html](admin/placement-exam.html)** — اختبارات تحديد المستوى (3 اختبارات) + معالج إنشاء/تعديل من 4 خطوات (نافذة ملء الشاشة)،
  و**admin/placement/exam-1…3.html** — تفاصيل الاختبار: إحصاءات ومحاولات المتدربين.
- **[admin/course-categories.html](admin/course-categories.html)** — تصنيفات الدورات كشجرة (15 تصنيفاً، الأسهم تطوي/تفتح الأبناء) + درج إضافة/تعديل التصنيف،
  و**admin/categories/category-{id}.html** — صفحة كل تصنيف (إحصاءات + البرامج التابعة له).
- **[admin/virtual-classrooms.html](admin/virtual-classrooms.html)** — الفصول الافتراضية (3 قاعات) + درج إنشاء القاعة،
  و**admin/vclasses/class-1…3.html** — صفحة القاعة بـ 8 تبويبات (معلومات، المدربون، الجلسات، المشاركون، التسجيلات، الملفات، الإحصائيات، سجل النشاط) مع درجي إضافة مدرب وجدولة جلسة.
- الأزرار التي تشير إلى صفحات غير مبنية بعد (سجل CRM، صفحة برنامج…) تعرض تنبيهاً «هذه الصفحة قيد الإنشاء».
...
✅ منجزة أيضاً: التعلم — `waitlist.html`، `placement-exam.html`، `course-categories.html`، `virtual-classrooms.html` (+ صفحات العرض)
⏳ متبقية — الاختبارات: `question-bank.html`،
