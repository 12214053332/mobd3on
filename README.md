...
├── trainee/                    ⏳ لم يبدأ بعد
├── crm/                        ⏳ لم يبدأ بعد
├── website/                    ✅ منجزة (إدارة الموقع الإلكتروني — 10 صفحات قائمة + صفحات Workspace، تفصيلها أدناه)
├── components/
...
│   │   ├── assess.js           (بحث وفلاتر متعددة المفاتيح على نفس الجدول/الشبكة — «الاختبارات وبنك الأسئلة»، «الشهادات»، «الإيرادات»، والمكتبة الإعلامية/المدونة في «إدارة الموقع»)
...
### الاختبارات وبنك الأسئلة + الشهادات (مجموعة «الاختبارات وبنك الأسئلة» في السايدبار)

- **[admin/question-bank.html](admin/question-bank.html)** — بنك الأسئلة: 4 بنوك أسئلة (توزيع صعوبة، عدد اختيار متعدد/صح-خطأ) + نافذة إنشاء/تعديل بنك،
  و**admin/qbank/bank-{id}.html** — أسئلة البنك: بحث + فلاتر النوع والصعوبة + نافذة إضافة/تعديل سؤال (اختيار متعدد أو صح/خطأ).
- **[admin/exams.html](admin/exams.html)** — الاختبارات (4 اختبارات) + معالج إنشاء/تعديل من 4 خطوات (معلومات الاختبار، اختيار بنك الأسئلة، توزيع الأسئلة، المعاينة والنشر)،
  و**admin/exams/exam-{id}.html** — تبويبا «المعاينة» و«النتائج» لكل اختبار.
- **[admin/grading.html](admin/grading.html)** — التصحيح والنتائج: مؤشرات + فلاتر (النتيجة/الحالة) + بحث، وحوار «إعادة الاختبار»،
  و**admin/grading/attempt-{id}.html** — تفاصيل المحاولة للقراءة فقط (سؤال بسؤال) + سجل المحاولات السابقة.
- **[admin/certificates.html](admin/certificates.html)** — الشهادات بتبويبين: «الشهادات الممنوحة» (5 شهادات، مؤشرات وفلاتر حالة) و«القوالب» (3 قوالب)،
  وزر «إضافة شهادة» يفتح نافذة اختيار بين إنشاء قالب (معالج من 4 خطوات) أو إصدار شهادة يدوي لمتدرب، وحوارا تأكيد للإلغاء وإعادة الإصدار.
  و**admin/certificates/{id}.html** — تفاصيل الشهادة (معاينة + سجل + بيانات الإصدار) مع رابط لصفحة تحقق عامة **admin/certificates/verify-{id}.html**.
...
### الإيرادات (مجموعة «الإيرادات» في السايدبار)

- **[admin/reports.html](admin/reports.html)** — التقارير (منجزة سابقاً ضمن المرحلة الأولى).
- **[admin/payments.html](admin/payments.html)** — المدفوعات: لوحة مالية (إجمالي الإيرادات، معاملات مكتملة/معلقة/فاشلة...)، بحث وفلتر حالة (9 حالات)، جدول 9 عمليات،
  ونوافذ: إضافة عملية دفع يدوياً، طلب استرداد، اعتماد/رفض تحويل بنكي، رابط دفع آمن.
  زرا «التقارير المالية» و«الكوبونات» في الرأس ينقلان إلى الصفحتين التاليتين.
  و**admin/payments/{TXN-id}.html** — مساحة عمل العملية بستة تبويبات (نظرة عامة، الفاتورة، الإيصال، الاسترداد، سجل النشاط، المرفقات والملاحظات)،
  وحوار رفض طلب الاسترداد ضمن تبويب الاسترداد.
  و**admin/payments/reports.html** — تقارير مالية (الإيرادات حسب الشهر، طرق الدفع، الكوبونات المستخدمة) مع أزرار تصدير.
- **[admin/coupons.html](admin/coupons.html)** — كوبونات الخصم: 7 كوبونات، بحث وفلتر حالة، نافذة إنشاء كوبون بملء الشاشة (9 أقسام: معلومات، نوع الخصم، النطاق، الأهلية، الحدود، الفترة، الرسالة الترويجية، الظهور، المراجعة)،
  ودرج تفاصيل الكوبون (4 تبويبات: المعلومات، التحليلات، سجل الاستخدام، محاكاة الخروج) وزر «تعديل» فيه يفتح نافذة الإنشاء، وحوار تأكيد الحذف.

### النمو والمحتوى — إدارة الموقع الإلكتروني (مجموعة فرعية `website/*`، شقيقة لـ `admin/`)

10 صفحات قائمة رئيسية + صفحات Workspace لكل عنصر، بنفس نمط بقية المشروع (تبويبات `.sec-tab`/`[data-panel]`، درجات تصميم فقط
تُفتح وتُغلق، بيانات الـ Prototype كما هي). الروابط العابرة لوحدات أخرى (الدورات، إدارة المستخدمين، CRM، الشهادات) تفتح الصفحة
الفعلية إن كانت مبنية، وإلا تنبيهاً «هذه الصفحة قيد الإنشاء».

- **[website/dashboard.html](website/dashboard.html)** — لوحة الموقع: 10 مؤشرات (حالة الموقع، الإصدار المنشور، تغييرات مسودة، صحة SEO...)،
  إجراءات سريعة، صفحات عُدّلت/نُشرت مؤخراً، وصحة الموقع (SEO ناقص، صور مفقودة، روابط معطّلة...).
- **[website/home.html](website/home.html)** — بنّاء الصفحة الرئيسية: قائمة أقسام الصفحة الرئيسية (11 قسماً) على اليسار — كل قسم تبويب حقيقي
  (`data-tab` على عنصر القسم نفسه بدل زر `.sec-tab` منفصل) يبدّل لوحة الإعداد المقابلة على اليمين، مع شريط سطح المكتب/لوحي/جوال.
  الأقسام المرتبطة بوحدة أخرى (البرامج، المدربون، الشهادات...) تعرض ملاحظة «للقراءة فقط، عدّل من الوحدة الأصلية».
- **[website/programs.html](website/programs.html)** — عرض 5 برامج (تُدار بياناتها من وحدة التعلم، هذه الصفحة تتحكم بالعرض والترتيب والظهور فقط)،
  و**website/programs/{id}.html** — مساحة عمل البرنامج بـ 7 تبويبات (معلومات البرنامج، إعدادات الصفحة، SEO، الظهور بالموقع، النماذج المرتبطة، معرض الوسائط، سجل التعديلات).
- **[website/forms.html](website/forms.html)** — نماذج الموقع العام (7 نماذج تجمع بيانات فقط وتُنشئ عملاء محتملين في CRM)، ومعالج إنشاء/تعديل نموذج من 3 خطوات،
  و**website/forms/{id}.html** — مساحة عمل النموذج (التحليلات، الطلبات، التضمين والروابط).
- **[website/pages.html](website/pages.html)** — صفحات الموقع الثابتة (9 صفحات: من نحن، تواصل معنا، سياسة الخصوصية...)، ومعالج إنشاء/تعديل صفحة من 4 خطوات (محرر نصوص Rich Text تصميمي)،
  و**website/pages/{id}.html** — مساحة عمل الصفحة (المحتوى، SEO، الوسائط، النماذج، النشر، سجل الإصدارات، التحليلات).
- **[website/media.html](website/media.html)** — المكتبة الإعلامية المركزية (9 ملفات: صور/فيديو/PDF/مستندات)، عرض شبكة أو قائمة (تبديل حقيقي)،
  مجلدات وفلاتر نوع حقيقية، درج رفع ملفات،
  و**website/media/{id}.html** — مساحة عمل الملف (المعاينة والتفاصيل، مُستخدم في، سجل الإصدارات) — الحذف محظور إن كان الملف مستخدَماً.
- **[website/blog.html](website/blog.html)** — المدونة والأخبار (6 عناصر: مقال/خبر/فعالية/قصة نجاح...)، فلتر نوع حقيقي، معالج إنشاء محتوى من 4 خطوات،
  و**website/blog/{id}.html** — مساحة عمل المحتوى بـ 8 تبويبات (المحتوى، SEO، الوسائط، محتوى ذو صلة، التحليلات، التعليقات، سجل الإصدارات، النشر).
- **[website/navigation.html](website/navigation.html)** — التنقل والقوائم (4 قوائم: الرئيسية، الفوتر، الجوال، لوحة الطالب)، درج إضافة قائمة، وحوار تأكيد الحذف،
  و**website/navigation/{id}.html** — مُنشئ القائمة: شجرة عناصر حقيقية (طي/فتح الفروع)، لوحة عناصر متاحة من محتوى الموقع القائم (لا تكرار يدوي)،
  معاينة حية (سطح مكتب/لوحي/جوال)، ودرج إعدادات العنصر؛ القوائم المخصصة للفوتر والجوال تعرض مُنشئ أعمدة/مُنشئ جوال مخصصاً بدل الشجرة.
- **[website/policies.html](website/policies.html)** — السياسات والامتثال (13 سياسة جاهزة، عرض/تعديل فقط — لا إنشاء أو حذف)، بحث حي،
  ودرجا «عرض» (معاينة الصفحة المنشورة) و«تعديل» (محرر Rich Text) لكل سياسة.
- **[website/settings.html](website/settings.html)** — إعدادات الموقع بـ 4 تبويبات: معلومات عامة (الاسم والشعار)، التواصل والروابط (وسائل التواصل الاجتماعي)،
  الهوية البصرية (ألوان أساسية/ثانوية/مميزة، تدرّج تلقائي، تباين WCAG، أنماط جاهزة، معاينة حيّة معزولة للمكوّنات)، والتحليلات (GA4، Meta Pixel، GTM).
...
- ✅ التحويل إلى Tailwind لم يغيّر أي مظهر: تمت مقارنة الـ computed styles وأبعاد كل عنصر (≈60 ألف عنصر في 98 صفحة) قبل/بعد على 8 عروض شاشة (1400 → 400px)، وفي حالات التفاعل (open/on/active/hover/focus...) — صفر فروقات، باستثناء انحراف ~4px غير ملحوظ بصرياً في `.cert-preview::before` على 5 صفحات شهادات (تم قبوله بعد المعاينة).
...
✅ منجزة أيضاً: الاختبارات وبنك الأسئلة — `question-bank.html`، `exams.html`، `grading.html`، `certificates.html` (+ صفحات العرض)
✅ منجزة أيضاً: الإيرادات — `payments.html`، `coupons.html` (+ صفحات العرض والتقارير المالية)
✅ منجزة أيضاً: النمو والمحتوى — `website/*` (إدارة الموقع الإلكتروني، تفصيلها في القسم أعلاه)
(+ شاشات تفاصيل/Workspace تُبنى مع كل قسم)
